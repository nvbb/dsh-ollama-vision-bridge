...
'		// primary: the vision-bridge section of the DSH settings document (hot-reloaded)',
'		let doc = null;',
'		try {',
...
'async function visionBridge(ctx, content, currentModel) {',
...
'	if (imageBlocks.length === 0) return { durable, note: null, model };',
'	const notes = [];',
...
'		const desc = await ollamaVisionDescribe({ ...cfg, model }, dataUri);',
...
'\t\t\t\t\t\t\t\tconst bridged = await visionBridge(ctx, content, current.model);',
'\t\t\t\t\t\t\t\tif (bridged !== null) {',
