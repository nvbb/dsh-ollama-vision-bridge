...
/** Replacement for the refusal statement (5 lines -> 19 lines, tab indented).
 * The user's own message keeps the image blocks and their text untouched; the
 * VL description is delivered as a separate context-injection message (source
 * kind "plugin"), so the transcript shows it as a collapsible 上下文注入 row
 * and the model still receives it. */
...
'\t\t\t\t\t\t\t\t\t\t\tcontent: [{ type: "text", text: `[\u56fe\u7247\u8bf4\u660e\uff08\u672c\u5730\u89c6\u89c9\u6a21\u578b ${bridged.model} \u751f\u6210\uff09]\\n${bridged.note}\\n\\n\uff08\u7cfb\u7edf\u8bf4\u660e\uff1a\u7528\u6237\u9644\u52a0\u4e86\u56fe\u7247\u3002\u5f53\u524d\u6a21\u578b\u4e0d\u652f\u6301\u539f\u751f\u56fe\u7247\u8bc6\u522b\uff0c\u4ee5\u4e0a\u63cf\u8ff0\u7531\u672c\u5730\u89c6\u89c9\u6a21\u578b ${bridged.model} \u751f\u6210\u3002\u8bf7\u5728\u56de\u7b54\u5f00\u5934\u7b80\u8981\u8bf4\u660e\u8fd9\u4e00\u70b9\uff0c\u7136\u540e\u57fa\u4e8e\u56fe\u7247\u5185\u5bb9\u56de\u7b54\u7528\u6237\u7684\u95ee\u9898\u3002\uff09` }],',
'\t\t\t\t\t\t\t\t\t\tsource: { kind: "plugin", plugin: "dsh-ollama-vision-bridge" }',
'\t\t\t\t\t\t\t\t\t});',
'\t\t\t\t\t\t\t\t\tif (mode === "steer") { agent.steer(message); agent.inject(injection); }',
'\t\t\t\t\t\t\t\t\telse { agent.followup(injection); agent.followup(message); }',
'\t\t\t\t\t\t\t\t} else {',
